    ...
    <div>
        ...
        <form onSubmit={handleCreate} className="mt-6 rounded-lg border border-neutral-800 p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={formName}
              onChange={(e) => { setFormName(e.target.value); setCreateError('') }}
          ...
          <div>
            <label className="block text-sm text-neutral-400 mb-1">Fallback chain (comma-separated model strings)</label>
            <input
              type="text"
              value={formChain}
              onChange={(e) => { setFormChain(e.target.value); setCreateError('') }}
      ...
      {!loading && configs.length === 0 && !showForm && (
        <div className="mt-8 rounded-lg border border-neutral-800 p-12 text-center">
          ...
          <p className="text-sm text-neutral-600 mt-1">
            Create a fallback chain so requests automatically retry with a different model on failure.
          </p>
        </div>
      )}
      ...
      {!loading && configs.length > 0 && (
        <div className="mt-8 space-y-3">
          {configs.map((config) => (
            <div key={config.id} className="flex items-start justify-between rounded-lg border border-neutral-800 p-4">
              <div>
