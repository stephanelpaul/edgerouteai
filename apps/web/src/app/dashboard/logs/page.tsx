...
export default function LogsPage() {
	...
	const loadLogs = useCallback(async () => {
		...
		try {
			...
			const d = data as { error?: { message?: string }; logs?: LogEntry[] }
			if (!r.ok) throw new Error(d.error?.message ?? `Error ${r.status}`)
			setLogs(d.logs ?? [])
		} catch (err) {
			setError((err as { message?: string }).message ?? 'Failed to load logs')
		} finally {
			setLoading(false)
		}
	}, [apiUrl])
					...
					<tbody>
						...
						{!loading &&
							logs.map((log) => (
								<tr
									key={log.id}
									className="border-t border-neutral-800/50 hover:bg-neutral-900/30 transition"
								>
									...
									<td className="px-4 py-3 text-neutral-400">${(log.cost ?? 0).toFixed(6)}</td>
									<td className="px-4 py-3 text-neutral-400">
