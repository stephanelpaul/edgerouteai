...
import { createDb, requestTransforms, apiKeys } from '@edgerouteai/db'
...
requestTransformsRoute.post('/', async (c) => {
  ...
  if (body.type === 'set_parameter') {
    try {
      ...
      throw new EdgeRouteError('value must be valid JSON for set_parameter type', 'validation_error', 400)
    }
  }
