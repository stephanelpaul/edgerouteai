...
adminRoute.put('/users/:id/role', superadminOnly, async (c) => {
  ...
  if (userId === c.get('userId')) {
    return c.json({ error: { message: 'Cannot change your own role', code: 'validation_error' } }, 400)
  }
...
adminRoute.get('/stats', adminOnly, async (c) => {
  const userCount = await c.env.DB.prepare('SELECT COUNT(*) as count FROM "user"').first<{ count: number }>()
  const keyCount = await c.env.DB.prepare('SELECT COUNT(*) as count FROM api_keys WHERE revoked_at IS NULL').first<{ count: number }>()
