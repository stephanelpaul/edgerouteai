...
export default function AliasesPage() {
  ...
  const loadAliases = useCallback(async () => {
    ...
    try {
      ...
      setAliases((data as any).aliases ?? [])
    } catch (err: any) {
        ...
        <form onSubmit={handleCreate} className="mt-6 rounded-lg border border-neutral-800 p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={aliasName}
              onChange={(e) => { setAliasName(e.target.value); setCreateError('') }}
      ...
      {!loading && aliases.length === 0 && !showForm && (
        <div className="mt-8 rounded-lg border border-neutral-800 p-12 text-center">
          ...
          <p className="text-sm text-neutral-600 mt-1">
            Create an alias to use short names like &quot;my-fast&quot; instead of full model identifiers.
          </p>
        </div>
      )}
