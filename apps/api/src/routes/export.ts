...
exportRoute.get('/logs', async (c) => {
	...
	// CSV format
	const header =
		'timestamp,provider,model,input_tokens,output_tokens,cost_usd,latency_ms,status_code'
...
exportRoute.get('/stats', async (c) => {
	...
	for (const l of filtered) {
		...
		if (!byDate[date])
			byDate[date] = { requests: 0, inputTokens: 0, outputTokens: 0, costUsd: 0, totalLatency: 0 }
