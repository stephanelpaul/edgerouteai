...
export default function AdminPage() {
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error((data as any).error?.message ?? 'Failed to update role')
      }
  ...
  const handleDelete = async (userId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        throw new Error((data as any).error?.message ?? 'Failed to delete user')
      }
  ...
  if (isLoading || loadingData) {
    return (
      <div className="p-8 text-neutral-400 text-sm">Loading...</div>
    )
  }
  ...
  if (error) {
    return (
      <div className="p-8">
        ...
        <button onClick={fetchData} className="mt-4 text-sm text-neutral-400 underline">Retry</button>
      </div>
    )
  }
    ...
    <div className="p-8 max-w-6xl">
      ...
      <div className="bg-neutral-900 rounded-lg border border-neutral-800 overflow-hidden">
        <div className="px-6 py-4 border-b border-neutral-800 flex items-center justify-between">
          ...
          <button onClick={fetchData} className="text-xs text-neutral-400 hover:text-white transition">Refresh</button>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-neutral-800 text-left">
                ...
                {isSuperadmin && <th className="px-6 py-3 text-xs text-neutral-500 font-medium">Actions</th>}
              </tr>
            </thead>
            <tbody>
              {users.map((u) => {
                ...
                return (
                  <tr key={u.id} className="border-b border-neutral-800/50 hover:bg-neutral-800/30 transition">
