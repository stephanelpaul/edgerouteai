...
authRoute.post('/sign-up/email', async (c) => {
  try {
    ...
    if (!name || !email || !password) {
      return c.json({ error: { message: 'name, email, and password are required', code: 'validation_error' } }, 400)
    }
    ...
    return c.json({ error: { message: 'Internal error during sign-up', code: 'internal_error' } }, 500)
  }
})
...
authRoute.post('/sign-in/email', async (c) => {
  try {
    ...
    return c.json({ error: { message: 'Internal error during sign-in', code: 'internal_error' } }, 500)
  }
})
...
authRoute.post('/sign-out', async (c) => {
  try {
    ...
    return c.json({ error: { message: 'Internal error during sign-out', code: 'internal_error' } }, 500)
  }
})
