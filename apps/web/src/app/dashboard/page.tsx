...
export default function DashboardPage() {
	...
	useEffect(() => {
		...
		fetch(`${apiUrl}/api/stats?days=7`, { credentials: 'include' })
			.then((r) => r.json())
			.then((data) => {
				const d = data as { error?: { message?: string }; stats?: Stats }
				if (d.error) throw new Error(d.error.message ?? 'Failed to load stats')
				setStats(d.stats ?? (data as Stats))
			})
		...
		<div>
				...
				<div className="mb-6 rounded-lg border border-amber-500/30 bg-amber-500/10 px-5 py-4 flex items-start justify-between">
					<div>
						...
						<p className="text-sm text-amber-400/80 mt-0.5">
							You are the first user — you have been granted superadmin access. Set up your provider
							keys to get started.
						</p>
					</div>
					<button
						onClick={() => setShowWelcomeBanner(false)}
						className="ml-4 text-amber-400/60 hover:text-amber-300 text-lg leading-none"
					>
						&times;
					</button>
				</div>
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-3">
					...
					<button
						onClick={() => {
							window.location.href = `${apiUrl}/api/export/stats?format=csv&days=30`
						}}
			...
			{!loading && !error && (
				<>
					<div className="mt-8 grid grid-cols-3 gap-6">
						<StatCard
							title="Total Spend"
							value={stats ? `$${(stats.totalCost ?? 0).toFixed(4)}` : '$0.00'}
						/>
