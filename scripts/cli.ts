...
async function doCreateUser(name: string, email: string, password: string, role = 'user') {
	...
	const userCount = countMatch ? Number.parseInt(countMatch[1], 10) : 1
	const effectiveRole = userCount === 0 ? 'superadmin' : role

	// Create Better Auth user
	d1Execute(
		`INSERT OR IGNORE INTO "user" ("id", "name", "email", "emailVerified", "role", "createdAt", "updatedAt") VALUES ('${userId}', '${name}', '${email}', 1, '${effectiveRole}', ${now}, ${now})`,
	)

	// Create Better Auth credential account
	d1Execute(
		`INSERT OR IGNORE INTO "account" ("id", "accountId", "providerId", "userId", "password", "createdAt", "updatedAt") VALUES ('${accountId}', '${userId}', 'credential', '${userId}', '${hashedPassword}', ${now}, ${now})`,
	)

	// Also create in our users table (for API key references)
	d1Execute(
		`INSERT OR IGNORE INTO "users" ("id", "email", "name", "created_at") VALUES ('${userId}', '${email}', '${name}', ${now})`,
	)
	...
	d1Execute(
		`INSERT INTO "api_keys" ("id", "user_id", "name", "key_hash", "key_prefix", "created_at") VALUES ('${keyId}', '${userId}', 'Default Key', '${keyHash}', '${keyPrefix}', ${now})`,
	)
...
async function createUser() {
	...
	if (!email || !password) {
		console.error(
			'Usage: pnpm cli create-user --email <email> --password <password> [--name <name>] [--role <role>]',
		)
		process.exit(1)
	}
...
async function resetPassword() {
	...
	d1Execute(
		`UPDATE "account" SET "password" = '${hashedPassword}', "updatedAt" = ${now} WHERE "userId" = '${userId}' AND "providerId" = 'credential'`,
	)
	...
	try {
		...
		if (jsonMatch) {
			...
			for (const row of rows) {
				...
				console.log(
					`  ${row.id}  ${row.email.padEnd(25)} ${(row.name || '-').padEnd(10)}  ${(row.role || 'user').padEnd(12)} ${date}`,
				)
			}
		}
	} catch {
...
async function createKey() {
	...
	d1Execute(
		`INSERT INTO "api_keys" ("id", "user_id", "name", "key_hash", "key_prefix", "created_at") VALUES ('${keyId}', '${userId}', '${name}', '${keyHash}', '${keyPrefix}', ${now})`,
	)
...
async function deploy() {
	...
	console.log('\n🌐 Building dashboard...')
	execSync(
		'cd apps/web && NEXT_PUBLIC_API_URL=https://edgerouteai-api.remediumdev.workers.dev npx next build',
		{ stdio: 'inherit' },
	)

	console.log('\n📄 Deploying dashboard...')
	execSync(
		'npx wrangler pages deploy apps/web/out --project-name edgerouteai-web --branch main --commit-dirty=true',
		{ stdio: 'inherit' },
	)
